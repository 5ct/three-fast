 ...
 * 模型炸裂
 */
export class ParticleEngine extends Base {
    constructor(scope) {
        super(scope);
    ...
    init(data = {}) {
        ...
        const self = this, scene = self.$scope.Scene, Type = Object.freeze({ "CUBE": 1, "SPHERE": 2 });
        self.Type = Type;
        let Tween = function (timeArray, valueArray) {
        ...
        let ParticleEngine = function () {
            ...
            this.particleMaterial = new THREE.ShaderMaterial({
                ...
                vertexShader: self.particleVertexShader,
                fragmentShader: self.particleFragmentShader,
                transparent: true,
        ...
        ParticleEngine.prototype.initialize = function () {
            ...
            let positions = [], colors = [], alives = [], opacitys = [], sizes = [], angles = [];
            for (let i = 0; i < this.particleCount; i++) {
                // remove duplicate code somehow, here and in update function below.
                this.particleArray[i] = this.createParticle();
                positions.concat(this.particleArray[i].position);
                alives.concat(this.particleArray[i].alive);
                colors.concat(this.particleArray[i].color);
                opacitys.concat(this.particleArray[i].opacity);
                sizes.concat(this.particleArray[i].size);
                angles.concat(this.particleArray[i].angle);
            }
            ...
            this.particleGeometry.setAttribute('alives', new THREE.Float32BufferAttribute(alives, 3));
            this.particleGeometry.setAttribute('opacitys', new THREE.Float32BufferAttribute(opacitys, 3));
            this.particleGeometry.setAttribute('angles', new THREE.Float32BufferAttribute(angles, 3));
            ...
            console.info(this.particleMesh);
        ...
        ParticleEngine.prototype.update = function (dt) {
            ...
            this.particleGeometry.position = new THREE.Float32BufferAttribute(positions, 3);
            this.particleGeometry.color = new THREE.Float32BufferAttribute(colors, 3);
            this.particleGeometry.position = new THREE.Float32BufferAttribute(alives, 3);
            this.particleGeometry.position = new THREE.Float32BufferAttribute(opacitys, 3);
            this.particleGeometry.position = new THREE.Float32BufferAttribute(angles, 3);
            this.particleGeometry.position = new THREE.Float32BufferAttribute(sizes, 1).setUsage(THREE.DynamicDrawUsage);
            // check if particle emitter is still running
            if (!this.emitterAlive)
                return;
            // if no particles have died yet, then there are still particles to actilete
            if (this.emitterAge < this.particleDeathAge) {
                // determine indices of particles to actilete
            ...
            for (let j = 0; j < recycleIndices.length; j++) {
                ...
                this.particleArray[i] = this.createParticle();
                this.particleArray[i].alive = 1.0; // actilete right away
        ...
        self.ParticleEngine = ParticleEngine;
    }
    render() {
    }
}
